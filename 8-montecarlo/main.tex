...
\path(c2|-I.north) ++(0,-1) node(A)[anchor=north,align=left]{
  ...
  \color{lila} \textit{Octubre 6, 2025}
};
\normalsize
...
\path(cp|-T.south) ++(.2,-1) node(B)[textof]{
 ...
 \hspace{-18mm}\color{celeste} $\ds \text{Prob}\ab[X\leq b]\equiv P(x)=\int_{x}^{b}\rho(x) \odif{x}$\\[3mm]
 \hspace{-18mm}$\ds \text{Prob}\ab[a<X\leq b]=P(b)-P(a)$\\[3mm]
};
...
\path(A.south west) ++(0,-.2) node(A)[verde,titulo,anchor=north west]{Demostración};

\path(A.south west) ++(0,-.3) node(A)[textof,anchor=north west,text width=5cm,align=left]{
 {\color{celeste} Caso Discreto}
 \setlength{\mathindent}{0cm}
 \begin{equation*}\begin{split}
 \ds\avg{f(X)}&=\sum^M_if(x_i)\dfrac{M_i}{M}\\[2mm]
 \ds&{\color{amarillo}=}\sum^N_if(x_i)P(x_i)\\
    &=\ab<f(X)>
 \end{split}\end{equation*}
};
 
\draw[amarillo](A.west) ++(0.6,-0.2) node{\small$(M\to\infty)$};

% \draw[->,ultra thick,celeste](A.north east) ++(-1.6,-1.1) -- ++(1,2) node[pos=1,above]{Discreto};

\path(A.north-|hr) ++(-0.8,0) node(A)[textof,anchor=north,text width=7cm,align=left]{
 {\color{celeste} Caso Continuo }\textit{\color{rosa}- discretizo con h}\\[-3.1mm]
 \setlength{\mathindent}{0cm}
 \begin{equation*}
 \dfrac{M_i}{M}{\color{amarillo}\approx} \color{rosa} \text{Prob}[x_i\leq X\leq x_i+h]\approx\rho(x_i)h
 \end{equation*}
 \begin{equation*}
 \begin{split}
 &\avg{f(X)}\approx\sum^{N_h}_if(x_i)\rho(x_i)h\\
 %FORMULA TRAPECIOS
%  &\approx\sum^N_if(X_i)\rho(X_i)\dfrac{b-a}{N}\\
 &\approx\int_a^bf(x_i)\rho(x_i)dx\quad {\color{rosa}h\to0}\\
 \end{split}\end{equation*}
};
\draw[amarillo](A.west) ++(1.6,1.5) node{\small$(M\to\infty)$};
...
\path(T.south) ++(0,-.5) node(A)[align=left,anchor=north]{
 Notese que  ${\ds\sum_i^L X^{(i)}=\sum_i^L X_i}$
};
            
\path(A.south) ++(0,-.5) node(A)[align=left,anchor=north]{
 Si usamos ${\ds X=\dfrac{f(X')}{L\rho(X')}\quad\rightarrow\quad Y=\sum_i^L
 \dfrac{f(X'_i)}{L\rho(X'_i)}=\int_{\Omega}f(x)\odif{x}}$
};
...
\path(t) ++(-.7,.7) node(T)[anchor=north west,text width=12cm]{\\[1mm]
Entonces aplicando el teorema
\begin{equation*}
  \ds\ab< Y>=L\ab<\dfrac{1}{L}\dfrac{f(X)}{\rho(X)}>=\ab<\dfrac{f(X)}{\rho(X)}>=\int f(x) \odif{x}
\end{equation*}
y además
\begin{equation*}
  \ds\sigma\ab( Y)=\sqrt{L}\sigma\ab(\dfrac{1}{L}\dfrac{f(X)}{\rho(X)})=\sigma\ab(\dfrac{f(X)}{\rho(X)})\dfrac{1}{\sqrt{L}}
\end{equation*}
Como $Y$ sigue una distribución normal, con cierta confianza\\[1mm]
\begin{equation*}
  \ds \ab< Y>={\color{amarillo}\avg{\color{fg}Y}}\pm t\dfrac{\sigma\ab(Y)}{\sqrt{\color{amarillo}M'}}{\color{amarillo}\,=\,} Y\pm t\sigma\ab(Y)
\end{equation*}
\begin{equation*}
  {\color{verde}\int f(x) \odif{x}}=\sum_i^L \dfrac{f(X_i)}{L\rho(X_i)}\pm t\sigma\ab(\dfrac{f(X)}{\rho(X)})\dfrac{1}{\sqrt{L}}=\color{verde}\avg{\ab(\dfrac{f(X)}{\rho(X)})}+O\ab(\dfrac{1}{\sqrt{M}}) 
\end{equation*}
};

\draw[amarillo](vl) ++(-0.4,1) node{\small$(M'\leftarrow 1)$};
...
\path(vu) ++(0,-.3) node(A)[textof,anchor=north]{
 $\ds\int_{\Omega}f(x)\odif{x}\approx\avg{\ab(\dfrac{f(X)}{\rho(X)})}$\ \ \ con \ $O\ab(\dfrac{1}{\sqrt{M}})$
};
...
\begin{scope}[x=1cm,y=1cm,shift=(scope),thick]
  ...
  function{0.008747*x*x};  % 3/(.2**3-7**3)
...
\path<2>(vl) ++(0,.2) node(A)[anchor=north]{
 Si {\color{verde}$\rho(x)=-cx^2$}, $\quad\ds\int_a^bf(x)\odif{x}\sim-\avg{e^{-X}}/c$
};
...
\path<1-3>(vl) ++(-1.3,4.1) node(A)[textof,anchor=north]{
...
% \begin{scope}[rotate=20,scale=0.8,every node/.style={scale=0.8}]
% \path<2>(cp-|np) ++(0,-.2) node(T)[def,anchor=north,align=left,text width=10cm]{\\[1mm]
%   $h(x)$ es cota superior de ${f(x)}$ en $a$ si existe ${\delta>0}$ y ${M>0}$ tal que:\\
%   \hspace{1cm} ${\ab|f(x)|\leq M \ab|h(x)|\quad\quad \forall |x-a|\leq \delta}$,\\
%   decimos que $f(x)$ es de orden de $h(x)$ en $a$ y se escribe:\\
%   \hspace{1cm} ${f(x)=O\ab(h(x))}\quad\quad \text{cuando} x\to a $};
% \path<2>(T.north west) node(H)[scale=0.8,defhead,anchor=west]{Notación de Landau / O-grande};
% \path<2>(T.north east) node(H)[scale=0.8,theohead,anchor=east,fill=black]{\color{white}\it en un punto $a$};
% \end{scope}                      
...
\path(cp) coordinate(media); 
\zfmedia{peli/buffon}{7cm}

\path(vid.south west) ++(0,0) node(A)[anchor=north west]
