...
\path(c2|-I.north) ++(0,-1) node(A)[anchor=north,align=left]{
  ...
  \color{lila} \textit{Agosto 15, 2025}
};
\normalsize
...
{Para un algoritmo iterativo ($n$ es iteración)}\\[2mm]
 \hspace{1cm}  {\small \color{lila} ABSOLUTO:} \hspace{1cm} $\ds\epsilon_n=\left|x_n-L\right|$\\[2mm]
 \hspace{1cm}  {\small \color{lila} RELATIVO:} \hspace{1cm} $\ds\tilde\epsilon_n=\left|x_n-L\right|/x_n$\\
};
                        
\path(A.south west) ++(0.,-.5) node(A)[align=left,anchor=north west,text width=10cm]{
 Para una aproximación discreta\\[2mm]
 \hspace{1cm}  {\small \color{rosa} TRUNCAMIENTO: \textit{\color{celeste}- LOCAL}} \\[2mm]
${\ds{\int_x^{x+h} f(x)dx=\frac{h}{2}\ab(f(x+h)+f(x))+\color{rosa}O\ab(h^3)}}$
};
...
\path(A.south west) ++(0.,-.5) node(A)[align=left,anchor=north west]{
 ...
 \hspace{1cm} ${\pi=3.141592653589793+\subnode{h}{\color{amarillo}O(\epsilon_M)}}$\\[1mm]
