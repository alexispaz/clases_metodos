...
\path(c2|-I.north) ++(0,-1) node(A)[anchor=north,align=left]{
  ...
  \color{lila} \textit{Septiembre 3, 2025}
};
\normalsize
...
\path<-3>(cp) ++(0.,-.2) node(A)[anchor=north,align=center]{
¿Resolvamos el PVI con ${\ds \phi_t'(0)=t}$?
};                                  
\path<2,3>(A.south-|t) ++(0.,-.3) node(A)[anchor=north west,align=left, execute at begin node=\setlength{\baselineskip}{4ex}]{
¡Un momento! Si ${\psi(x)}$ es solución\ldots también ${\psi_t(x)=c\psi(x)}$. \\
${\therefore}$ existe $c_t$ tal que  ${\ds \phi_t'(0)=\psi_t'(0)}$ y entonces $\phi_t$ es solución.\\
};


\path<2,3>(A.south-|cp) ++(0.,0) node(A)[anchor=north,align=center]{
 \Large\color{celeste}Cualquier disparo resuelve el PVF.
};
                                          
\path<3->(x.south) ++(-3,-.3) node(z)[ellipse,anchor=north,align=center,draw,celeste,fill=black,ultra thick]{
Partícula confinada \\ cualquier $V(x)$
};                                        
\path<3->(y.east) ++(.3,0) node[inner sep=1,circle,anchor=east,align=center,draw,celeste,fill=black,ultra thick]{
$\checkmark$
};
         
\draw<4->[amarillo,thick,->](z.south) -- ++(0,-.5) node[pos=1,below]{¡Siempre! es numérico};
                             
                             
\path<5->(ne) ++(-.4,-.3) node[rotate=-5,anchor=north east,align=center,amarillo]{
¡Nos interesa \\ conocer E!
};                                  
                                          

\path<5->(A.south-|cp) ++(0.,-2.5) node(A)[anchor=north,align=left, execute at begin node=\setlength{\baselineskip}{4ex}]{
...
\path(np|-t) ++(-.5,-2) node(A)[align=center]{
  Es parcial\ldots pero EDO1 en t, y con $\Psi(x,0)$ es un PVI};
                    
\path(A.south) ++(.5,-.5) node(A)[anchor=north,align=left, execute at begin node=\setlength{\baselineskip}{4ex}]{
  {\color{naranja} Opción 1:} Euler, RK4, Leap-Frog, Crank-Nicolson\ldots\\[3mm]
...
{\color{naranja} Opción 3:} ${\ds\Psi(x,0)=\sum_{n}c_n\color{celeste}\psi_n(x)}$\\[2mm]
${\hspace{2cm}\ds \Rightarrow\Psi(x,t)=\sum_{n}c_n\color{celeste}\psi_n(x)\color{rosa}e^{-i E_n t/\hbar}}$};
