...
\path(c2|-I.north) ++(0,-1) node(A)[anchor=north,align=left]{
  ...
  \color{lila} \textit{Agosto 13, 2025}
};
\normalsize
...
\path<1>(t-|np) ++(0,.2) node(T)[def,anchor=north,align=left,text width=10cm]{\\[1mm]
  $h(x)$ es cota superior de $f(x)$ si existe $x_0$ y ${M>0}$ tal que:\\
  \hspace{1cm} ${\ab|f(x)|\leq M \ab|h(x)|\quad\quad\forall x\geq x_0}$,\\
  entonces decimos que $f(x)$ es de orden de $h(x)$ y se escribe:\\
  \hspace{1cm} ${f(x)=O\ab(h(x))}\quad\quad x\to\infty $};
\path<1>(T.north west) node(H)[defhead,anchor=west]{Notación de Landau / O-grande};
\path<1>(T.north east) node(H)[theohead,anchor=east,fill=black]{\color{white}\it en infinito};
 
\path(cp-|np) ++(0,-.2) node(T)[def,anchor=north,align=left,text width=10cm]{\\[1mm]
...
\path(T.north west) node(H)[defhead,anchor=west]{Notación de Landau / O-grande};
\path(T.north east) node(H)[theohead,anchor=east,fill=black]{\color{white}\it en un punto $a$};
        
\path<2>(T.north west) ++(0.,1) node[align=left,anchor=south west]{
...
\path<2>(ep) ++(0,1) node[rotate=30,anchor=south east]{\includegraphics[width=6mm]{marcadores.png}};
...
\path(sp) ++(0.,1) node(A)[anchor=south,align=center]{
  {\color{celeste}¿Interpretación geométrica?} Ver interpolación polinómica de Lagrange.\\
  \hfill\small\color{celeste}\textit{(al final de la clase)}
};
...
\path(A.south west) ++(0.,-.5) node(A)[align=left,anchor=north west,text width=8cm]{
 ...
 \hspace{1cm}  {\small \color{rosa} TRUNCAMIENTO:} \\[2mm]
  \hspace{2cm} ${f'(x)=\frac{f(x+h)-f(x)}{h}+\color{rosa}O\ab(h)}$\\[1mm]
};

\path(A.south west) ++(0.,-.5) node(A)[align=left,anchor=north west]{
 Por usar una compu \\[2mm]
 \hspace{1cm}  {\small \color{amarillo} PRECISIÓN / NUMÉRICO:} \\[2mm]
 \hspace{1cm} ${\pi=3.141592653589793\subnode{h}{\color{amarillo}\;\pm\;\epsilon_M}}$\\[1mm]
};        
                          
\path(hr) ++(.5,-3) node{${\epsilon_M=2^{-52}\approx 2.22\,10^{-16}}$};

\draw[amarillo,<-,dashed](h) to[bend right] ++(1,-1) node[right]{Épsilon de la máquina};
...
podemos propagar el error numérico de ${f(x+h)-f(x)}$ como $\pm\;2\epsilon_M$\\
Entonces el {\color{verde}error total} se puede estimar como $\sim 2\epsilon_M/h+ch$.
};

% NOTA, XXX, TODO: Si hacemos propagacion de errores en el cociente
% entonces si z=\ab(f(x+h)-f(x))/h)
% error de z/\ab(f(x+h)-f(x))/h)=2epsilon/(f(x+h)-f(x)) + epsilon/h
% error de z=\ab(f(x+h)-f(x))/h) * [ 2epsilon/(f(x+h)-f(x)) + epsilon/h ]
% error de z=2epsilon/h + epsilon*\ab(f(x+h)-f(x))/h**2 
% error de z=2epsilon/h + epsilon*z/h
% Ademas se puede poner explicto c en el error de
% truncamiento como hacen en el apunte
...
% \begin{zframe}{}
% TODO 2026 Ejercicio para reducir el error de truncamiento
% 
...
\path(np) ++(0,-1) node(T)[ej,anchor=north,text width=10cm,align=justify]{\vspace{2mm}\\
\small Luego del seminario del departamento, Abril no esta muy convencida
acerca de la explicacion de Felipe sobre el corrimiento del pico de absorbancia
de un espectro de Kryptonita 21. Ella sospecha que el equipo Uv-vis está mal
calibrado. Por lo tanto decide reproducir el experimento unas 1245 veces más
para colectar estadistica suficiente sobre la variacion de la posicion del pico
y determinar si existe algun error sistemático en la medición. Sin embargo necesita
procesar estos espectros de forma automatica. \\
\small¿Puede usted ayudar a Abril?
};
